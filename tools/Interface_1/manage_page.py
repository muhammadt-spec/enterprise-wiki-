...
class manage_page(Tool):
        ...
        for txt in labels_texts:
            ...
            for lid, lab in labels_tbl.items():
                if str(lab.get("space_id")) == str(space_id) and str(lab.get("label_text")).lower() == str(txt).lower():
                    ...
                    break
            if not lid_found:
                new_id = manage_page._gen_id(labels_tbl)
                labels_tbl[new_id] = {
                    "label_id": new_id,
                    "label_text": txt,
                    "scope": "space",
    ...
    def invoke(
        ...
        if action == "create":
            ...
            sid = manage_page._space_id_by_key(spaces, space_key)
            if not sid:
                return json.dumps({"error": f"space_key {space_key} not found"})

            pid = manage_page._gen_id(pages)
            rec = {
                "page_id": pid,
                "space_id": sid,
                "title": title,
                ...
                "last_modified_at": manage_page._now(),
                "status": "current",
                "workflow_state": None,
                "restrictions_id": None,
                "version_number": 1,
                "created_at": manage_page._now(),
                "updated_at": manage_page._now()
            }
            pages[pid] = rec

            if labels:
                label_ids = manage_page._ensure_labels(data, labels, sid, creator_id)
                for lid in label_ids:
                    pl_id = manage_page._gen_id(page_labels)
                    page_labels[pl_id] = {
                        "page_label_id": pl_id,
                        "page_id": pid,
                        "label_id": lid,
                        "applied_at": manage_page._now()
                    }
            ...
            rec["updated_at"] = manage_page._now()
            rec["last_modified_at"] = manage_page._now()
            return json.dumps(rec)
