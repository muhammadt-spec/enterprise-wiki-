...
class manage_calendar(Tool):
    ...
    def invoke(
        ...
        if action == "create":
            ...
            sid = manage_calendar._space_id_by_key(spaces, space_key)
            if not sid:
                return json.dumps({"error": f"space_key {space_key} not found"})
            cid = manage_calendar._gen_id(calendars)
            rec = {
                "calendar_id": cid,
                "space_id": sid,
                "name": calendar_name,
                "created_by": None,
                "created_at": manage_calendar._now(),
                "updated_at": manage_calendar._now()
            }
            calendars[cid] = rec
        ...
        if action == "add_event":
            ...
            eid = manage_calendar._gen_id(events)
            rec = {
                "event_id": eid,
