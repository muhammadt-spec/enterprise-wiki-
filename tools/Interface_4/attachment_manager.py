...
class attachment_manager(Tool):
    ...
    def invoke(
        ...
        if action == "upload":
            ...
            aid = attachment_manager._gen_id(attachments)
            rec = {
                "attachment_id": aid,
                "page_id": page_id,
                ...
                "uploaded_at": attachment_manager._now(),
                "version_number": 1,
                "created_at": attachment_manager._now(),
                "updated_at": attachment_manager._now()
            }
            attachments[aid] = rec
        ...
        if action in ["replace","move","delete"]:
            ...
            if action == "move":
                ...
                att["updated_at"] = attachment_manager._now()
                return json.dumps(att)
