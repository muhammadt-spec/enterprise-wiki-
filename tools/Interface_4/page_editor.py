...
class page_editor(Tool):
        ...
        for txt in labels_texts:
            ...
            for lid, lab in labels_tbl.items():
                if str(lab.get("space_id")) == str(space_id) and str(lab.get("label_text")).lower() == str(txt).lower():
                    ...
                    break
            if not lid_found:
                new_id = page_editor._gen_id(labels_tbl)
                labels_tbl[new_id] = {
                    "label_id": new_id,
                    "label_text": txt,
                    "scope": "space",
    ...
    def invoke(
        ...
        if action == "create":
            ...
            sid = page_editor._space_id_by_key(spaces, space_key)
            if not sid:
                return json.dumps({"error": f"space_key {space_key} not found"})

            pid = page_editor._gen_id(pages)
            rec = {
                "page_id": pid,
                "space_id": sid,
                "title": title,
                ...
                "last_modified_at": page_editor._now(),
                "status": "current",
                "workflow_state": None,
                "restrictions_id": None,
                "version_number": 1,
                "created_at": page_editor._now(),
                "updated_at": page_editor._now()
            }
            pages[pid] = rec

            if labels:
                label_ids = page_editor._ensure_labels(data, labels, sid, creator_id)
                for lid in label_ids:
                    pl_id = page_editor._gen_id(page_labels)
                    page_labels[pl_id] = {
                        "page_label_id": pl_id,
                        "page_id": pid,
                        "label_id": lid,
                        "applied_at": page_editor._now()
                    }
            ...
            rec["updated_at"] = page_editor._now()
            rec["last_modified_at"] = page_editor._now()
            return json.dumps(rec)
