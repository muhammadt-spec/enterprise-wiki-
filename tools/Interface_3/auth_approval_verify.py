...
class auth_approval_verify(Tool):
    @staticmethod
    def _now_iso() -> str:
    ...
    def invoke(
        ...
        if scope and scope.lower() == "space_key":
            ...
            sid = auth_approval_verify._find_space_id_by_key(spaces, space_key)
            if not sid:
                return json.dumps({
                    "approval_status": "invalid",
        ...
        result = {
            ...
            "scope": scope,
            "space_key": space_key,
            "timestamp": auth_approval_verify._now_iso()
        }
