...
class file_manage(Tool):
    ...
    def invoke(
        ...
        if action == "upload":
            ...
            aid = file_manage._gen_id(attachments)
            rec = {
                "attachment_id": aid,
                "page_id": page_id,
                ...
                "uploaded_at": file_manage._now(),
                "version_number": 1,
                "created_at": file_manage._now(),
                "updated_at": file_manage._now()
            }
            attachments[aid] = rec
        ...
        if action in ["replace","move","delete"]:
            ...
            if action == "move":
                ...
                att["updated_at"] = file_manage._now()
                return json.dumps(att)
