...
class restriction_set_update(Tool):
    @staticmethod
    def _now()->str:
    ...
    def invoke(
        ...
        rs_id = restriction_set_update._gen_id(rs_tbl)
        rs_tbl[rs_id] = {"restrictions_id": rs_id, "type": restriction_type, "created_at": restriction_set_update._now(), "updated_at": restriction_set_update._now()}
        pages[str(page_id)]["restrictions_id"] = rs_id
        pages[str(page_id)]["updated_at"] = restriction_set_update._now()

        added_users = []
        for uid in (subjects.get("users") or []):
            if str(uid) not in users:
                return json.dumps({"error": f"user {uid} not found"})
            rid = restriction_set_update._gen_id(rsu_tbl)
        ...
        for gid in (subjects.get("groups") or []):
            ...
            rid = restriction_set_update._gen_id(rsg_tbl)
