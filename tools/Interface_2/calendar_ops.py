...
class calendar_ops(Tool):
    ...
    def invoke(
        ...
        if action == "create":
            ...
            sid = calendar_ops._space_id_by_key(spaces, space_key)
            if not sid:
                return json.dumps({"error": f"space_key {space_key} not found"})
            cid = calendar_ops._gen_id(calendars)
            rec = {
                "calendar_id": cid,
                "space_id": sid,
                "name": calendar_name,
                "created_by": None,
                "created_at": calendar_ops._now(),
                "updated_at": calendar_ops._now()
            }
            calendars[cid] = rec
        ...
        if action == "add_event":
            ...
            eid = calendar_ops._gen_id(events)
            rec = {
                "event_id": eid,
