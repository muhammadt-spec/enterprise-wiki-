...
class GetsPagesInSpace(Tool):
    ...
    def invoke(data: Dict[str, Any], space_key: str, status: Optional[str] = None,
        ...
        for p in pages.values():
            if p.get("space_id") != sid:
                continue
            if status and p.get("status") != status:
                continue
            if workflow_state and p.get("workflow_state") != workflow_state:
                continue
            results.append(p)
    ...
    def get_info():
        return {
            "type":"function",
            "function":{
                "name":"gets_pages_in_space",
                "description":"List pages within a space, with optional status/workflow filters.",
                "parameters":{
                    "type":"object",
                    "properties":{
                        "space_key":{"type":"string","description":"Space key"},
                        "status":{"type":"string","description":"Filter by status (Draft | Published | Archived)"},
                        "workflow_state":{"type":"string","description":"Filter by workflow_state (Proposed | In_Review | Approved | Rejected | Retired)"}
                    },
                    "required":["space_key"]
                }
            }
