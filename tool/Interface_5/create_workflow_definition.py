...
class CreateWorkflowDefinition(Tool):
    ...
    def invoke(data: Dict[str, Any], name: str, scope: str, steps: Dict[str, Any],
        ...
        space_id=None
        if scope=="Space":
            space_id=CreateWorkflowDefinition._find_space_id_by_key(spaces, space_key)
            if not space_id:
                return json.dumps({"error":"Space not found for workflow scope"})
        ...
        new_id = CreateWorkflowDefinition._generate_id(workflows)
    ...
    def get_info():
        return {
            ...
            "function":{
                ...
                "description":"Create a workflow with steps (JSON) at Global or Space scope.",
                "parameters":{
                    "type":"object",
                    "properties":{
                        "name":{"type":"string","description":"Workflow name"},
                        "scope":{"type":"string","description":"Workflow scope (Global | Space)"},
                        "steps":{"type":"object","description":"Steps JSON payload for the workflow"},
                        "enabled":{"type":"boolean","description":"Enable workflow (True/False)"},
                        "space_key":{"type":"string","description":"Required if scope is Space: space key"}
                    },
