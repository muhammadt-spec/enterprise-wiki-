...
class UploadAttachmentForPage(Tool):
    ...
    def invoke(data: Dict[str, Any], page_id: str, filename: str, mime_type: str,
        ...
        if uploader_id not in users:
            return json.dumps({"error":"Uploader user not found"})
        new_id = UploadAttachmentForPage._generate_id(attachments)
    ...
    def get_info():
        return {
            "type":"function",
            "function":{
                "name":"upload_attachment_for_page",
                "description":"Upload a new attachment to a page.",
                "parameters":{
                    "type":"object",
                    "properties":{
                        "page_id":{"type":"string","description":"Target page_id"},
                        "filename":{"type":"string","description":"File name"},
                        "mime_type":{"type":"string","description":"MIME type string"},
                        "filesize_bytes":{"type":"integer","description":"File size in bytes"},
                        "uploader_id":{"type":"string","description":"User ID of uploader"},
                        "version_number":{"type":"integer","description":"Attachment version number (integer)"}
                    },
