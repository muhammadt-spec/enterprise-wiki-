...
class ConfigureWorkflowBlueprint(Tool):
    ...
    def invoke(data: Dict[str, Any], name: str, scope: str, steps: Dict[str, Any],
        ...
        space_id=None
        if scope=="Space":
            space_id=ConfigureWorkflowBlueprint._find_space_id_by_key(spaces, space_key)
        ...
        new_id = ConfigureWorkflowBlueprint._generate_id(workflows)
    ...
    @staticmethod
    def get_info():
        return {"type":"function","function":{"name":"configure_workflow_blueprint","description":"Create a workflow with steps (JSON) at Global or Space scope.","parameters":{"type":"object","properties":{"name":{"type":"string","description":"Workflow name"},"scope":{"type":"string","description":"Workflow scope (Global | Space)"},"steps":{"type":"object","description":"Steps JSON payload for the workflow"},"enabled":{"type":"boolean","description":"Enable workflow (True/False)"},"space_key":{"type":"string","description":"Required if scope is Space: space key"}},"required":["name","scope","steps"]}}}
