...
class CreateCiRecord(Tool):
    ...
    def invoke(data: Dict[str, Any], name: str, ci_type: str, environment: str, status: str,
        ...
        new_id = CreateCiRecord._generate_id(cis)
    ...
    @staticmethod
    def get_info():
        return {"type":"function","function":{"name":"create_ci_record","description":"Create a new configuration item (CI).","parameters":{"type":"object","properties":{"name":{"type":"string","description":"Unique CI name"},"ci_type":{"type":"string","description":"CI type (App | Service | Server | DB | Network | Storage | Endpoint | Other)"},"environment":{"type":"string","description":"Environment (Prod | Staging | Dev | Test)"},"status":{"type":"string","description":"CI status (Active | Maintenance | Retired)"},"service_id":{"type":"string","description":"Linked service_id"},"owner_group_id":{"type":"string","description":"Owner group_id"}},"required":["name","ci_type","environment","status"]}}}
