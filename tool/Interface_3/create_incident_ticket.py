...
class CreateIncidentTicket(Tool):
    ...
    def invoke(data: Dict[str, Any], title: str, description_code: str, status: str, priority: str,
        ...
        new_id = CreateIncidentTicket._generate_id(incidents)
    ...
    @staticmethod
    def get_info():
        return {"type":"function","function":{"name":"create_incident_ticket","description":"Create a new incident with enum-validated fields.","parameters":{"type":"object","properties":{"title":{"type":"string","description":"Incident title"},"description_code":{"type":"string","description":"Controlled description code"},"status":{"type":"string","description":"Incident status (New | Assigned | In_Progress | Resolved | Closed)"},"priority":{"type":"string","description":"Priority (P1 | P2 | P3 | P4 | P5)"},"impact":{"type":"string","description":"Impact (Low | Medium | High | Critical)"},"urgency":{"type":"string","description":"Urgency (Low | Medium | High | Critical)"},"reporter_id":{"type":"string","description":"Reporter user_id"},"assignee_id":{"type":"string","description":"Assignee user_id"},"assignment_group_id":{"type":"string","description":"Assignment group_id"},"category":{"type":"string","description":"Category (e.g., Hardware | Software | Network | Access | Security)"},"subcategory":{"type":"string","description":"Subcategory code"},"ci_id":{"type":"string","description":"Configuration item id"},"problem_id":{"type":"string","description":"Related problem id"},"service_id":{"type":"string","description":"Related service id"},"resolution_notes_code":{"type":"string","description":"Controlled resolution code"}},"required":["title","description_code","status","priority","impact","urgency","reporter_id"]}}}
