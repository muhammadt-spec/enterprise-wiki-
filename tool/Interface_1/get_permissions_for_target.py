...
class GetPermissionsForTarget(Tool):
    """
    ...
    def invoke(data: Dict[str, Any], target_id: str) -> str:
        ...
        for psid, ps in permission_sets.items():
            if ps.get("target_id") == target_id:
                grants = [g for k,g in permission_grants.items() if g.get("permission_set_id") == psid]
    ...
    def get_info():
        return {
          "type": "function",
          "function": {
            "name": "get_permissions_for_target",
            "description": "List permission sets and grants for a target entity (space/page/global).",
            "parameters": {
              "type": "object",
              "properties": {
                "target_id": {"type": "string", "description": "Target entity id (space_id or page_id or global target id)"}
              },
              "required": ["target_id"]
            }
          }
