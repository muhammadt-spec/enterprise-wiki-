...
class AddCalendarEvent(Tool):
    ...
    def invoke(
        ...
        new_id = AddCalendarEvent._generate_id(events)
        rec = {
            "event_id": new_id,
            "calendar_id": calendar_id,
            "title": title,
            "start_at": start_at,  # (YYYY-MM-DD)
            "end_at": end_at,      # (YYYY-MM-DD)
    ...
    def get_info():
        return {
          ...
          "function": {
            ...
            "description": "Create a new event on an existing calendar.",
            "parameters": {
              "type": "object",
              "properties": {
                "calendar_id": {"type": "string", "description": "Target calendar_id"},
                "title": {"type": "string", "description": "Event title"},
                "start_at": {"type": "string", "description": "Start date (YYYY-MM-DD)"},
                "end_at": {"type": "string", "description": "End date (YYYY-MM-DD)"},
                "event_type": {"type": "string", "description": "Event type (Meeting | Milestone | Release | Holiday | Other)"},
                "created_by": {"type": "string", "description": "User ID who creates the event"}
              },
              "required": ["calendar_id", "title", "start_at", "end_at", "event_type"]
            }
          }
        }
