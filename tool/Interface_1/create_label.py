...
class CreateLabel(Tool):
    ...
    def invoke(
        ...
        for lid, lv in labels.items():
            if (lv.get("scope") == scope and
                (lv.get("space_id") or None) == (space_id or None) and
                (lv.get("label_text") or "").lower() == label_text.lower()):
                return json.dumps({"error": "Label already exists in scope"})

        new_id = create_label._generate_id(labels)
        rec = {
            "label_id": new_id,
